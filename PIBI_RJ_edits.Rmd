...
author:
  ...
  - name: "With contributions from Rikke Jepsen, rjepsen@icprb.org"
    affilication: Interstate Commission on the Potomac River Basin
output:
  html_document:
    df_print: paged
    toc: true
...
The R-scripts contained in this document describe the method used to calculate the PIBI, following the methodology outlined by Lacouture et al. (2006), with additional references from documentation written by former ICPRB staff member, Jacqueline M. Johnson. The code chunks are designed to be copied and pasted into the RStudio editor. In order to keep the scripts organized, it is suggested that the corresponding section header is added and commented out using `#`. The phytoplankton data used are extracted from the Chesapeake Bay Program's Chesapeake Environmental Data Repository (CEDR) using an Application Programming Interface (API).
...
1. Create a new folder with any name you choose. This will house the main directory structure.
...
```
####**Custom Functions** 
First establish the `clean_string()` function. It modifies character objects by 1) removing leading and trailing white spaces, 2) converting all characters to lowercase, and 3) replacing all spaces with an underscore. These corrections eliminate common typos and standardize character objects.
...
```
The process to identify water quality dates within the 3-day window of the phytoplankton sampling date can be lengthy. The `parallel` package is used to speed this up.
...
```
Create a unique ID for phytoplankton sampling events. The ID is a concatenation of the station and sample date.
```{r}
phyto.df <- phyto.df %>%
  mutate( sampledate =  as.Date(sampledate)) %>%
  mutate(concat = paste0(station, sampledate))
```
Create a unique ID for water quality events. The ID is a concatenation of the station and sample date.
```{r}
wq.df <- wq.df %>%
  mutate( sampledate =  as.Date(sampledate)) %>%
  mutate(concat = paste0(station, sampledate))
...
```
Determine if the phytoplankton sample depth is above the pdepth.
```{r}
...
```
Data with a depth at or below the pdepth is filtered out.
```{r}
...
di_s_chla.df <- s_chla.df %>%
  ...
  ungroup()
```
Export the entries with errors as a CSV ("cb3.3c_mutants.csv"). These are entries that are likely reporting errors stemming from a known issue with station cb3.3c between 1986 and 1991, where a second entry was taken that was vastly different from the first.
```{r}
...
wq_above_pdepth.df <- wq_above_pdepth.df %>%
  ...
  ungroup()
```
Join surface chloraphyll data to the water quality data.
```{r}
di_s_chla.df <- di_s_chla.df %>% 
...
NOTE: a message starting with "Joining, by = ..." may appear when this code chunk is run. This is not an error, and you can proceed with the scripts without issue.
```{r message=FALSE}
...
carbon.df <- data.table::fread(file.path(project.dir, "data/carbon/", "carbon_list_2014.csv")) %>% 
  clean_up()
```
Reformat the carbon data so that it will join with the previously-formatted data.
```{r warning=FALSE}
carbon.df <- carbon.df %>%
...
```
Import taxonomic data associated with phytoplankton data that was downloaded from CEDR.
```{r}
...
NOTE: a message stating "NAs introduced by coercion to integer range" may appear. This is not an error and you can proceed with the following scripts.
```{r message=FALSE}
bay.df <- taxa.raw %>% 
  filter(layer %in% c("ap", "wc")) %>% 
  distinct() %>% 
...
no.match.df <- anti_join(bay.df, carbon.df, by = c("latinname", "size")) %>% 
  ...
  distinct()
```
Try to find partial matches for taxa in `no.match.df`. Each row in `no.match.df` represents a unique taxon and size combination. The function,`lapply`, loops through each row, extracts the unique size string, splits the string into smaller pieces, and attempts to find partial matches with associated size strings in `carbon.df`. The string in `carbon.df` with the most partial matches will be used to replace the size string in `bay.df`.
```{r}
...
```
Remove columns that will be replaced with data from the `carbon.df` dataframe in the next step.
```{r}
bay.df <- bay.df %>%
...
```
Save the final scores to the "Scores/Ratings" folder within your directory.
```{r}
