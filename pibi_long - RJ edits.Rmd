...
This R-script and its accompanying documentation describe the method used to calculate the Phytoplankton Index of Biotic Integrity (PIBI) for the Chesapeake Bay and its tidal tributaries. Methodology was obtained from Lacouture et al. (2006), with additional references from Jacqueline M. Johnson’s documentation. Use this document to prepare data from the Chesapeake Bay Program's CEDR database and then calculate the PIBI. Data are extracted from the database using an (Application Programming Interface) API.

###**Before You Get Started**  
RStudio must be installed on your computer to implement the following R scripts. If RStudio is not installed, it can be downloaded for free from [https://www.rstudio.com/](https://www.rstudio.com/). This [webpage](https://courses.edx.org/courses/UTAustinX/UT.7.01x/3T2014/56c5437b88fa43cf828bff5371c6a924/) details the short installation process. Once R Studio is ready to be used on your computer, copy and paste the R scripts into the RStudio editor and run them. Each time a data frame is created, it can be viewed by typing `View(name)` in the console or by clicking its name in the Environment tab on the right.          
...
####**Setting Up R Document and Loading Packages** 
Open RStudio. Create a new RScript (File > New File > R Script), and save it in a location where you would like the scripts to be saved.      
...
The variables below represent values found in the Bay Program's [API guide](http://datahub.chesapeakebay.net/API). Unless changes occur to the API's structure, these variables will not need to be changed.

The water quality variable codes are as follows: 
chla = chlorophyll A
doc = dissolved organic carbon
pheo = pheophytin

#### ELABORTATE ON WHAT THESE MEAN AND SHOULD THE WQ PARAMETERS BE ""?
```{r}
...
salinity = 83
```
####**Establish Functions for Data Preparation and API Connection** 
...
The use of the dir.creat structure ensures that the proper directory structure is in place when pulling data using the API.
```{r}
...
```
####**Set the Date Range for the Desired Dataset**
Use `min_date` and `max date` to set the date range of the data you want to download. Note that ending date can be set to variable `todays.date` if you want to extract the most current data every time you run the download code. Also note that the format of the date input into these variables must match with the examples in place below (`mm-dd-yyyy`). 
```{r}
...
```
####**Data Pull Setup**
Now that the date range has been set, the required inputs for the data pull are established using `station.vec`.
```{r}
...
Perform the data pull from CEDR using the API. The data are saved in a dataframe called `phyto.df`. 
```{r}
...
```
####**Dataframe Creation**
Create a dataframe that contains picoplankton data. Note that the number above "station" will change from 17 to 18, because the ID for picoplankton is 18; phytoplankton is 17. The data are downloaded to the data/CEDR/ directory as a csv using `data.table::fwrite`.
```{r}
...
```
####**Download and Prepare Water Quality Data**
Water quality data need to be dowlnoaded next. This is done in a similar manner as the data pull just completed.
```{r}
...
data.table::fwrite(wq.df, file.path(project.dir, "data/water_quality", "cedr_wq.csv"))
```
Retrieve the data that was previously stored. 
```{r}
...
wq.df <- data.table::fread(file.path(project.dir, "data/water_quality/cedr_wq.csv"), data.table = FALSE, na.strings = c(""))
```
Rearrange the water quality data to match the phytoplankton data.
```{r}
# Create a unique ID for phytoplankton sampling events that is a concatenation of the sation and sample date. 
phyto.df <- phyto.df %>%
...
# Create a vector to store the concatinated ids from phytoplankton sampling events.
...
```
####**Prep Event Data**
Convert the sample dates from both phytoplankton data and the event data to date format.
```{r}
phyto.df <- phyto.df %>%
  mutate(sampledate = as.Date(sampledate))
...
```
Filter out all data outside of layers __above pycnocline__ and __water column__ and select only columns that will be joined.
```{r}
pdepth.df <- event.df %>%
...
di_s_chla.df <- s_chla.df %>%
  ...
  ungroup()
```
Write mutant entries to file water_quality\cb3.3c_mutants.csv. These are entries that are likely reporting errors and stems from a known issue with station cb3.3c between 1986 and 1991, where a second entry was taken that was vastly different from the first.
```{r}
...
```
Averge sample replicates for the rest of the data. 
```{r}
...
```
Assign a season value to dates between March-May and July-September.
```{r}
...
```
For samples missing salzone data, import salzones from Jackie Johnson's data and determine if her historical data can be assigned to any missing samples.
```{r}
...
```
####**Prep Carbon Data**
Import the carbon data and use the `clean_up()` function on it.
```{r}
...
```
Bring in taxanomic data associated with phytoplankton data that was downloaded from CEDR.
```{r}
...
mutate(sampledate = as.Date(sampledate))
```
Filter the data so that only samples from spring and summer and above the pycnocline are kept.
```{r}
bay.df <- taxa.raw %>% 
  filter(layer %in% c("ap", "wc")) %>% 
  distinct() %>% 
...
```
Assign Taxonomic Serial Numbers (TSNs) to taxa not found in the Integrated Taxonomic Information System (ITIS) database.
```{r}
bay.df <- bay.df %>% 
  mutate(
    tsn = as.integer(tsn),
...
carbon.dups <- carbon.df %>% 
  ...
  filter(count > 1)
```
Exclude three taxa that have known duplicates.
```{r}
carbon.df <- carbon.df %>% 
...
no.match.df <- anti_join(bay.df, carbon.df, by = c("latinname", "size")) %>% 
  ...
  distinct()
```
Find partial matches for taxa in *no.match.df*. Each row in *no.match.df* represents a unique taxon and size combination. The funtion,`lapply`, loops through each row, extracts the unique size string, splits the string into smaller pieces, and attempts to find partial matches with associated size strings in *carbon.df*. The string in *carbon.df* with the most partial matches will be used to replace the size string in *bay.df*.
```{r}
partial.match.df <- lapply(1:nrow(no.match.df), function(row.i) {
  ...
  bind_rows()
```
Merge *bay.df* with *partial.match.df*.
```{r}
...
```
Assigns TSNs with NA to be zero.
```{r}
carbon.df <- carbon.df %>%
...
bay_merged.df <- left_join(bay.df, carbon.df, by = c("latinname","size","tsn")) %>%
  distinct()
```
Turn carbon NAs into zeroes and calcualte nano_micro_biomass.  
```{r}
...
bay_wq_merged_calc.df <- bay_wq_merged_calc.df %>%
  ...
  distinct()
```
Apply metrics to all applicable rows. (WHAT'S THIS??")
```{r}
...
bay_wq_merged_calc.df <- bay_wq_merged_calc.df %>%
  ...
  ungroup()
```
Run a test to determine determine if the biomasses are adding up correctly. Open the *test* dataframe, and if it is not empty, your biomasses aren't ading up correctly.
```{r}
...
```
####**Generate Ratings**
Generate ratings based on the scores. Ratings can be poor, fair_poor, fair, fair_good, and good.
```{r}
scores_ratings_final <- scores_ratings_final %>%
