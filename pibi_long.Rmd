---
title: "pibi_long"
author: "Luke Vawter & Zachary Smith"
...
min_date = "1-1-2013"
#this is the minimum date to be included, add the variable todays.date intot his variable if you want the most recent data
max_date = "12-31-2016"
```

these variables references values found in the API guide at http://datahub.chesapeakebay.net 
```{r}
...
#thisvariable holds the url for the CEDR api
...
generate a new dataframe with new parameter s_chla (531 items, should be 533)
```{r}
...
bay_wq_merged_calc.df <-  bay_wq_merged_calc.df  %>%
  ...
  mutate(diatom_carbon = case_when(group == "diatom"~ nano_micro_biomass#/1000000
                                    , TRUE ~ as.numeric(NA))) %>%
  mutate(diatom_biomass = case_when(!is.na(diatom_carbon)~ sum(diatom_carbon, na.rm = T)#/1000000
                                    , TRUE ~ as.numeric(NA))) %>%
  
  
  mutate(dinoflagellate_carbon = case_when(group == "dinoflagellates"~ nano_micro_biomass#/1000000
                                    , TRUE ~ as.numeric(NA))) %>%
  mutate(dinoflagellate_biomass = case_when(!is.na(dinoflagellate_carbon)~ sum(dinoflagellate_carbon, na.rm = T)#/1000000
                                    , TRUE ~ as.numeric(NA))) %>%
  
  
  mutate(cyanophyte_carbon = case_when(group == "cyanophyte"| group == "cyanobacteria"~ nano_micro_biomass#/1000000
                                    , TRUE ~ as.numeric(NA))) %>%
  mutate(cyanophyte_biomass = case_when(!is.na(cyanophyte_carbon)~ sum(cyanophyte_carbon, na.rm = T)#/1000000
                                    , TRUE ~ as.numeric(NA))) %>%
  
  
  mutate(cryptophyte_carbon = case_when(group == "cryptophyte" | group == "cryptophyceae" | group == "cryptophyta" | group == "cryptomonads"~ nano_micro_biomass#/1000000
                                    , TRUE ~ as.numeric(NA))) %>%
  ...
  mutate(percent_cryptophyte_biomass = cryptophyte_biomass/total_nano_micro_biomass * 100) %>% #total_nano_micro_biomass)#%>%
  
  select(-group, -carbon, -nano_micro_biomass) %>%
  distinct()
...
bay_wq_merged_calc.df <- bay_wq_merged_calc.df %>%
  ...
  #filter(unique_id %in% c("sbe5_ap_fs1_2013-03-13", "we4.2_ap_fs1_2013-03-20", "ret5.2_ap_fs1_2013-03-20")) %>%
  ...
  #mutate(diatom_biomass =( case_when(diatom_biomass == "-inf"~as.numeric(NA), TRUE~diatom_biomass))) %>%

  mutate(cyanophyte_biomass = max(cyanophyte_biomass, na.rm=T)) %>%
  #mutate(cyanophyte_biomass =( case_when(cyanophyte_biomass == -inf~as.numeric(NA), TRUE~cyanophyte_biomass))) %>%
  
  mutate(dinoflagellate_biomass = max(dinoflagellate_biomass , na.rm=T)) %>%
  #mutate(dinoflagellate_biomass =( case_when(dinoflagellate_biomass == "-inf"~as.numeric(NA), TRUE~dinoflagellate_biomass))) %>%
  
  #mutate(cyanophyte_biomass = sum(dinoflagellate_biomass, na.rm=T)) %>%
  mutate(cryptophyte_biomass = max(cryptophyte_biomass, na.rm=T)) %>%
  #mutate(cryptophyte_biomass =( case_when(cryptophyte_biomass == "-inf"~as.numeric(NA), TRUE~cryptophyte_biomass))) %>%
...
```{r}
#test_bay_wq_merged_calc.df[is.na(test_all_seasons)] <-
```
...
metrics_prep.df <- bay_full_calc.df %>%
  ...
  #rename(total_nano_micro_biomass = total_nano_micro_biomass) %>%
...
score_spring_f <- metrics_prep.df %>% 
  filter(
           ...
           # is.na(total_nano_micro_biomass_chla_ratio) == FALSE #|
           # is.na(surface_chla) == FALSE |
           # is.na(cyanophyte_biomass) == FALSE |
           # is.na(doc) == FALSE |
           # is.na(pheophytin) == FALSE |
           # is.na(total_nano_micro_biomass) == FALSE
           ) %>%
...
scores_ratings_final <- test_all_seasons%>%#test_all_seasons%>%
    # mutate(ibi_score =  case_when(season == "spring" & salzone == "f" ~ mean(total_nano_micro_biomass_chla_ratio_score, surface_chla_score, cyanophyte_biomass_score, doc_score, pheophytin_score, total_nano_micro_biomass_score, na.rm = T), TRUE~ as.numeric(NA)))
  
  #filter(unique_id == "ret5.2_ap_fs1_2013-03-20") %>%
...
surface_chla_score + pct_cryptophyte_score + diatom_biomass_score + dinoflagellate_biomass_score + doc_score + pheophytin_score + picoplankton_abundance_score + total_nano_micro_biomass_score) / 9,
                                                ...
                                                TRUE~ 0#as.numeric(NA)
  )) 

```

replace 0 values in scores with blanks for clarity
```{r}
