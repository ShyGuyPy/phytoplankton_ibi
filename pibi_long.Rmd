...
COMPARE STEP 4
note: 3477 to 3481(83?) in Claire's excel
```{r}
...
the objects in diff_check2.df are the 4 entries that exist in Claire's data but not in mine.  An exception needs to be written for such cases:
```{r}

#diff_check.df <- left_join(step_4_modified.df, s4_compare_modified.df)

diff_check2.df <- anti_join(s4_compare_modified.df, step_4_modified.df, by = c("station", "layer", "depth", "parameter", "measurevalue", "eventid", "pdepth")) 
...
generates new dataframe with new parameter s_chla (539 items, should be 533)
```{r}
...
STEP 5 COMPARE
my 539 to Claire's 533
```{r}
step_5.df <- s_chla.df

data.table::fwrite(step_5.df, file.path(project.dir, "data/test", "step_5.csv"))

s4_compare.df <- data.table::fread(file.path(project.dir, "data/test/", "C_step5.csv")) %>% clean_up() #%>% mutate(sampledate=as.Date(sampledate))
```
