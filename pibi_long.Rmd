...
test_min_date = "1-01-1970"

test_max_date = todays.date
```
...
dataframe to compare the non-functioning full date range data pull against the functioning partial date range data pull
```{r}
phyto_all.df <-  file.path(url.root,
                      "LivingResources",
                      "TidalPlankton",
                      "Reported",
                       test_min_date,
...
read saved data back into dataframe `wq.df`.  Runnning this code allows you to skip the downloading process in cases where your dataset is static.
```{r}
...
if (nrow(wq_need_sal.df) > 1) {
  ...
  wq_warm_sal_added.df <- left_join(wq_warm_sal_added.df, wq_has_sal.df, by= c("concat", "salzone"))
}
