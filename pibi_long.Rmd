...
wq_sal_test.df <- left_join(wq_warm.df, wq_sal_id.df, by = c("concat"))#, "salzone"))
...
find salinity blanks
```{r}
wq_sal_test_blank.df <- wq_sal_test.df %>%
  filter(is.na(salzone) | salzone == 0)
```


Assign F salzone to some station/season instances based on known salinity profiles
```{r}
...
wq_sal_test_fill.df <-  wq_sal_test.df %>%
...
test.df <- wq_sal_test_fill.df %>%
  filter(salzone == "test")

test_na.df <- wq_sal_test_fill.df %>%
