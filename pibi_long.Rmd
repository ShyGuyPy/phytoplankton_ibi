...
wq_w_pdepth.df <- wq_w_pdepth.df %>%
  mutate(above_pdepth = case_when(depth <= pdepth~FALSE,
                                  depth > pdepth~TRUE))
```
...
missing_pdepth.df <- wq_w_pdepth.df %>%
  filter( (is.na(pdepth)& station %in% c("tf3.3","tf4.2","tf5.5", "ret3.1","ret4.3","ret3.1")) #| 
            #(depth > pdepth & station %in% c("tf3.3","tf4.2","tf5.5", "ret3.1","ret4.3","ret3.1"))
)
...
COMPARE STEP 4
note: 3480 to 3483 in Claire's excel
```{r}
...
diff_check_step_4.df <- anti_join(s4_compare_modified.df, step_4_modified.df,  by = c("station", "layer", "depth", "parameter", "measurevalue", "eventid", "pdepth")) 
...
generates new dataframe with new parameter s_chla (540 items, should be 533)
```{r}
...
s_chla.df <- s_chla.df %>%
  ...
  #do not remove NAs
  mutate(measurevalue_average = mean(measurevalue)) %>%
  ungroup()

```

STEP 5 COMPARE
my 540 to Claire's 533
```{r}
step_5.df <- s_chla.df
...
s5_compare_modified.df <- s5_compare.df %>%
  rename(eventid = wq_eventid) %>%
  rename(measurevalue_average = average_of_measurevalue) %>%
  #rename(pdepth = p_depth) %>%
  #select(station, sampledate, layer, depth, parameter, measurevalue, eventid, pdepth, above_pdepth) %>%
  #select(-sampledate, -above_pdepth) %>%
  mutate(author = "Claire")
...
C_diff_check_step_5.df <- anti_join(step_5_modified.df,s5_compare_modified.df, by = c("station", "layer", "parameter", "measurevalue_average", "eventid")) 

LV_diff_check_step_5.df <- anti_join(s5_compare_modified.df, step_5_modified.df, by = c("station", "layer", "parameter", "measurevalue_average", "eventid")) 
```

this code makes exceptions for special cases found in diff_check_step5.df
(there are 16 exceptions, 9 are from Claire's while 7 are from Luke's document)
```{r}
...
```{r}
wq_rep_check <- wq_above_pdepth.df %>%
...
C_diff_check_step_6.df <- anti_join(step_6.df,s6_compare.df, by = c("station", "depth","parameter", "measurevalue_averaged")) 

LV_diff_check_step_6.df <- anti_join(s6_compare.df, step_6.df, by = c("station", "depth", "parameter", "measurevalue_averaged")) 
...
COMPARE STEP 7 
mine is 2027 to Claire's is 2024)
```{r}
...
```{r}
wq_all.df <- left_join(wq_above_pdepth.df, s_chla.df, by = c("an as yet to be determined varaible..."))
