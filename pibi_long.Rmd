...
bay_wq_merged_calc.df <-  bay_wq_merged.df %>%
  ...
  mutate(dinoflagellate_biomass = case_when(group == "dinoflagellate"~sum(carbon), TRUE ~ as.numeric(NA))) %>%
  mutate(cyanophyte_biomass = case_when(group == "cyanophyte"~sum(carbon), TRUE ~ as.numeric(NA))) %>%
  mutate(cryptophyte_biomass = case_when(group == "cryptophyte"~sum(carbon), TRUE ~ as.numeric(NA))) %>%
...
test <- bay_wq_merged_calc.df %>%
  filter(dinoflagellate_biomass != 0)
```
