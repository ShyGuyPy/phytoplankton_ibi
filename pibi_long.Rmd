...
bay_wq_merged_calc.df <- bay_wq_merged_calc.df %>%
  group_by(concat) %>%
  mutate(procentrum_minimum_abundance = case_when(latinname == "prorocentrum_minimum"~ sum(as.double(reportingvalue)),
                                                  TRUE~0)) %>%
  mutate(microcystis_aeruginosa_abundance = case_when(latinname == "microcystis_aeruginosa"~ sum(as.double(reportingvalue)), TRUE~0)) %>%
  ungroup()
```

join the picoplankton data into the bay data
```{r}
...
```{r}
metrics_test.long <- bay_full_calc.df %>%
...
```{r}
metrics.long <- metrics_test.long %>% 
...
Error in score_spring_f(metrics.long) : score_spring_f requires the following metric(s): total_phyto_biomass_chla_ratio, 
surface_chla,
cyanophyte_biomass, 
doc, 
pheophytin, 
...
score_spring_f <- function(metrics.long) {
  ...
  final.df <- metrics.sub %>% 
    mutate(score = case_when(
      ...
      #added by Luke
      metric == "percent_cryptophyte_biomass" & value > 7.1 ~ 1,
      metric == "percent_cryptophyte_biomass" & value <= 7.1 & value >4.9 ~ 3,
      metric == "percent_cryptophyte_biomass" & value <= 4.9 ~ 5,
...
call teh above function on our data
```{r}
score_test.df <- score_spring_f(metrics.long)
```

```{r}
test <- metrics.long %>%
  filter(metric == "total_phyto_biomass")
```
