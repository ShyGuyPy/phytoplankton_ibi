...
bay_wq_merged_calc.df <-  bay_wq_merged.df %>%
  ...
  mutate(diatom_biomass = case_when(group == "diatom"~sum(carbon), TRUE ~ as.numeric(NA))) %>%
  mutate(dinoflagellate_biomass = case_when(group == "dinoflagellates"~sum(carbon), TRUE ~ as.numeric(NA))) %>%
  mutate(cyanophyte_biomass = case_when(group == "cyanophyte"| group == "cyanobacteria"~sum(carbon), TRUE ~ as.numeric(NA))) %>%
  mutate(cryptophyte_biomass = case_when(group == "cryptophyte" | group == "cryptophyceae" | group == "cryptophyta"~sum(carbon), TRUE ~ as.numeric(NA))) %>%
  
  mutate(percent_cryptophyte_biomass = cryptophyte_biomass/total_biomass)%>%
...
scores_ratings_final.long <- all_seasons.rating %>%
  ...
  select(unique_id, salzone, station, sampledate, samplenumber, metric,value, score, ibi_score, rating) %>%
  distinct() %>%
...
```{r}
all_seasons.rating.wide <- all_seasons.rating %>%
  select(unique_id, station, sampledate, samplenumber, salzone, metric, value, score, ibi_score, rating) %>%
  select(-ibi_score, -rating, -score) %>%
  group_by(unique_id) %>%
  mutate(grouped_id = row_number())  %>%
