...
All other variables and functions will likely not need to be changed
...
It is recommendedthat you run the code from here if you've already pulled from the api.  It will save time to start from here.  You will need to run the variables and functions section again to populate them into your RStudio environment tab. 
CEDR
```{r}
...
downloading the water quality data is done in a similar manner as the event and sample data
```{r}
wq.df <- file.path(url.root,
                   ...
                   paste(station.vec, collapse = ","),
                   paste(
                     #these are the parameter variables declared in Funcs and Variables section
...
```
note: more information about programIds and projectIds canbe found at http://datahub.chesapeakebay.net/API
...
the below code reads saved dataframe back into wq.df.  Runnning this code allows you to skip the downloading process in cases where your dataset is static.
```{r}
...
we reduce the water quality data to match the phytoplankton data
```{r}
...
filter out all data outside of layers __above pycnocline__ and __water column__ and select only columsn we wish to join.
```{r}
pdepth.df <- event.df %>%
...
join selected event data to water quality data
```{r}
