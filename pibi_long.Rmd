...
bay_wq_merged_calc.df <- bay_wq_merged_calc.df %>%
  group_by(concat) %>%
  mutate(prorocentrum_minimum_abundance = case_when(latinname == "prorocentrum_minimum"~ sum(as.double(reportingvalue)),
                                                  TRUE~0)) %>%
...
test1 <- separate(spring.f, concat, c("station", "layer", "samplenumber",
                        "sampledate", "season", "salzone"), sep = "_",
           remove = FALSE) 

test2 <- separate(metrics.long, concat, c("station", "layer", "samplenumber",
                        "sampledate", "season", "salzone"), sep = "_",
           remove = FALSE) 

test3 <- separate(metrics.long, metric, c("station", "layer", "samplenumber",
                        "sampledate", "season", "salzone"), sep = "_",
           remove = FALSE) 

test4 <- separate(metrics.long, metric, metric.vec, sep = "_",
           remove = FALSE) 
print(metric.vec)

scores.df <- metrics.long %>% 
  mutate(unique_id = "temp") %>%
