...
bay_wq_merged_calc.df <-  bay_wq_merged_calc.df  %>%
  ...
  mutate(percent_cryptophyte_biomass = cryptophyte_biomass/total_nano_micro_biomass) %>% #total_nano_micro_biomass)#%>%
...
```{r}
test_scores_ratings_final <-  scores_ratings_final %>%
mutate



#test_scores_ratings_final[is.na(test_scores_ratings_final[total_nano_micro_biomass_chla_ratio_score:ibi_score]) ] <- " "
```

```{r}
zero <- function(x) {
  x = case_when(is.na(x)==T~0, TRUE~x)
}
```
...
test2_scores_ratings_final <- scores_ratings_final %>%
  filter(unique_id == "ret5.2_ap_fs1_2013-03-20") %>%
  mutate_at(.vars=vars(ends_with("_score"), list( 
 
 case_when(.==0 ~ "test", TRUE~.))
  #~case_when( #| is.na(.) == TRUE | . == NA 
            #.==0 ~ "test")
))#)
# 
