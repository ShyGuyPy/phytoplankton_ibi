...
```{r}
# event.df %>%
# data.table::fwrite(file.path(project.dir, "data/ScoresRatings", "event_with3day.csv"))
```
...
```{r}
# event.df %>%
# data.table::fwrite(file.path(project.dir, "data/ScoresRatings", "event_no3day.csv"))
```
...
test
```{r}
# problem <- bay_merged.df %>%
#   filter(station =="et3.1", sampledate ==as.Date("2010-09-20")) %>%
#   mutate(nano_micro_biomass = case_when(is.na(nano_micro_biomass) ~0, TRUE~nano_micro_biomass)
#                                ) %>%
#   mutate(total_nano_micro_biomass = sum(as.integer(nano_micro_biomass))) 
```
...
bay_wq_merged_calc.df <-  bay_wq_merged_calc.df  %>%
  ...
  mutate(total_nano_micro_biomass = sum(nano_micro_biomass, na.rm = TRUE)) %>%
  ...
  mutate(other_biomass_total = sum(other_biomass,na.rm = TRUE)) %>%
...
NA's must be changed to zeros to calculate properly
```{r}
