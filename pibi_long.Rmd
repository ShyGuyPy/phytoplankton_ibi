...
find_missing <- function(data_backbone, appended_data, modified_appended_data, step_vector) {
  missing.df <- anti_join(data_backbone, modified_appended_data, by = c("station","sampledate"))
  ...
  concat_vector2 <- pull(missing.df, concat)
  
  missing_full.df <-appended_data %>%
    filter(concat %in% concat_vector2) 
  
  modified_appended_data <- modified_appended_data %>%
  mutate( sampledate =  as.Date(sampledate)) %>%
  mutate(concat = paste0(station, sampledate))

  final.df <- full_join(modified_appended_data, missing_full.df, by = step_vector)
                          #c("station", "sampledate", "layer", "pdepth", "concat", "depth", "parameter", "measurevalue", "above_pdepth", "eventid"))
  
  return(final.df)
}
#show the mssing values
name_missing <- function(data_backbone, appended_data, modified_appended_data, step_vector) {
  missing.df <- anti_join(data_backbone, modified_appended_data, by = c("station","sampledate"))
...
di_s_chla.df <- s_chla.df %>%
  ...
  distinct(station, sampledate, depth, eventid, layer, parameter, measurevalue_averaged) %>%
  ungroup()
...
#wrong parameter somewhere I think
missing_s_chla <- find_missing(event.df, di_s_chla.df, di_s_chla.df, step_5.vec)
```


STEP 5 COMPARE
my 531 to Claire's 533
```{r}
