...
Simplest way to get Zach's code working was to add a org_tsn category that equals tsn(org_tsn is no longer a part of CEDR data output)
```{r}
# bay.temp <- bay.temp %>%
#   mutate(org_tsn = tsn)
```

Zach's original itis code
```{r}
...
```{r}
bay_partials_added_test.df <- bay_partials_added.df %>%
...
L. Make a vector of unique latinanems where size is not na... became pointless...solved another way
```{r}
# bay_w_size.df <- bay_partials_added.df %>% 
#   filter(!size == "not_applicable")
# 
# bay_w_o_size.df <- bay_partials_added.df %>% 
#   filter(size == "not_applicable") %>%
#   select(-size)
# #names_w_sized.vec <- unique(bay_reduced.df$latinname) 
# ```
# 
# ```{r}
# bay_w_size_merge.df <- left_join(bay_w_size.df, carbon.df, by = c("latinname", "size"))
# ```
# 
# ```{r}
# bay_w_o_size_merge.df <- left_join(bay_w_o_size.df, carbon.df, by = c("latinname"))
# ```
# 
# 
# ```{r}
# bw_missing_carbon.df <- bay_w_size_merge.df %>%
#   filter(is.na(carbon) | carbon == 0)
# 
# bwo_missing_carbon.df <- bay_w_o_size_merge.df %>%
#   filter(is.na(carbon) | carbon == 0)
# ```
# 
# 
# ```{r}
# bay_test.df <- bay_merged.df <- left_join(bay_w_o_size_merge.df, bay_w_size_merge.df, by = c("latinname", "size")) 
```
...
```{r}
bay_merged.df <- left_join(bay_partials_added_test.df, carbon.df, by = c("latinname", "size")) %>% 
...
note: missing tsn is 465(down from 756), missing carbon is 226(down from 9832)
