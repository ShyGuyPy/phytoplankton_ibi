...
test_all_seasons <- all_seasons %>%
  mutate(ibi_score =  case_when(season == "spring" & salzone == "f" ~mean(total_phyto_biomass_chla_ratio_rating, surface_chla_rating, cyanophyte_biomass_rating, cyanophyte_biomass_rating, doc_rating, pheophytin_rating, total_phyto_biomass_rating), TRUE~ as.numeric(NA)))
...
test <- test %>%  
  mutate(spice = nrow(total_phyto_biomass:picoplankton_abundance_rating)) %>%
  mutate(pepper = sum(total_phyto_biomass:picoplankton_abundance_rating))
```
