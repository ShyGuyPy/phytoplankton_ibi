...
bay_wq_merged_calc.df <-  bay_wq_merged_calc.df  %>%
  #filter(unique_id == "we4.2_ap_fs1_2014-08-06") %>%  
  #filter(unique_id == "ret5.2_ap_fs1_2013-07-15") %>%
  distinct() %>%
  group_by(unique_id) %>%
  ...
  mutate(diatom_carbon = case_when(group == "diatom"~ carbon#/1000000
                                    , TRUE ~ as.numeric(NA))) %>%
  mutate(diatom_biomass = case_when(!is.na(diatom_carbon)~ sum(diatom_carbon, na.rm = T)#/1000000
                                    , TRUE ~ as.numeric(NA))) %>%
  
  
  mutate(dinoflagellate_carbon = case_when(group == "dinoflagellates"~ carbon#/1000000
                                    , TRUE ~ as.numeric(NA))) %>%
  mutate(dinoflagellate_biomass = case_when(!is.na(dinoflagellate_carbon)~ sum(dinoflagellate_carbon, na.rm = T)#/1000000
                                    , TRUE ~ as.numeric(NA))) %>%
  
  
  mutate(cyanophyte_carbon = case_when(group == "cyanophyte"| group == "cyanobacteria"~ carbon#/1000000
                                    , TRUE ~ as.numeric(NA))) %>%
  mutate(cyanophyte_biomass = case_when(!is.na(cyanophyte_carbon)~ sum(cyanophyte_carbon, na.rm = T)#/1000000
                                    , TRUE ~ as.numeric(NA))) %>%
  
  
  mutate(cryptophyte_carbon = case_when(group == "cryptophyte" | group == "cryptophyceae" | group == "cryptophyta" | group == "cryptomonads"~ carbon#/1000000
                                    , TRUE ~ as.numeric(NA))) %>%
  mutate(cryptophyte_biomass = case_when(!is.na(cryptophyte_carbon)~ sum(cryptophyte_carbon, na.rm = T)#/1000000
                                    , TRUE ~ as.numeric(NA))) %>%
  
  select(-diatom_carbon, -dinoflagellate_carbon, -cyanophyte_carbon, -cryptophyte_carbon) %>%
  
  mutate(percent_cryptophyte_biomass = cryptophyte_biomass/total_biomass) %>% #total_nano_micro_biomass)#%>%
...
apply metrics to all rows applicable(CODE IS BRITTLE AND NEEDS A FIX)
```{r}
bay_wq_merged_calc.df <- bay_wq_merged_calc.df %>%
  group_by(unique_id) %>%
  ...
  mutate(dinoflagellate_biomass = sum(dinoflagellate_biomass , na.rm=T)) %>%
  ...
  #mutate(total_nan_micro_biomass = sum(total_nan_micro_biomass, na.rm=T)) %>% 
  #mutate(total_nan_micro_biomass_chla_ratio = sum(total_nan_micro_biomass_chla_ratio, na.rm=T)) %>%
  
  mutate(percent_cryptophyte_biomass = sum(percent_cryptophyte_biomass, na.rm=T)) %>%
...
```{r}
  metric.vec <- c("total_nan_micro_biomass_chla_ratio",
                  "surface_chla",
                  "chla",
                  "doc",
                  "pheophytin",
                  "total_nan_micro_biomass", 
                  "diatom_biomass",
...
metrics_prep.df <- bay_full_calc.df %>%
  ...
  #rename(total_nan_micro_biomass = total_nano_micro_biomass) %>%
  rename(total_nan_micro_biomass_chla_ratio = carbon_chlorophyll_ratio) %>%
...
metrics_prep.df <- metrics_prep.df %>%
  select(unique_id, station, sampledate, samplenumber, season, salzone, total_nan_micro_biomass_chla_ratio,
                  surface_chla,
                  chla,
                  doc,
                  pheophytin,
...
score_spring_f <- metrics_prep.df %>% 
  filter(
           ...
           # is.na(total_nan_micro_biomass_chla_ratio) == FALSE #|
           # is.na(surface_chla) == FALSE |
           # is.na(cyanophyte_biomass) == FALSE |
           # is.na(doc) == FALSE |
           # is.na(pheophytin) == FALSE |
           # is.na(total_nan_micro_biomass) == FALSE
           ) %>%
  mutate(total_nan_micro_biomass_chla_ratio_score = case_when(
      total_nan_micro_biomass_chla_ratio < 39.28 ~1,
      total_nan_micro_biomass_chla_ratio>= 39.28 & total_nan_micro_biomass_chla_ratio <= 41.72 ~ 3,
      total_nan_micro_biomass_chla_ratio > 41.72 ~ 5,
      TRUE ~ as.numeric(NA))) %>%
  ...
  mutate(total_nan_micro_biomass_score = case_when(
      total_nan_micro_biomass <= 172.99 | total_nan_micro_biomass > 828.5 ~ 1,
      total_nan_micro_biomass >= 583.9 & total_nan_micro_biomass <= 828.5 ~ 3,
      total_nan_micro_biomass > 172.99 & total_nan_micro_biomass < 583.9 ~ 5,
      TRUE ~ as.numeric(NA))) 
...
score_spring_o <- metrics_prep.df %>% 
  ...
  mutate(total_nan_micro_biomass_chla_ratio_score = case_when(
      total_nan_micro_biomass_chla_ratio < 18.8 | total_nan_micro_biomass_chla_ratio > 48.6 ~ 1,
      total_nan_micro_biomass_chla_ratio >= 18.8 & total_nan_micro_biomass_chla_ratio <= 21.2 ~ 3,
      total_nan_micro_biomass_chla_ratio > 21.2 & total_nan_micro_biomass_chla_ratio < 48.6 ~ 5,
      TRUE ~ as.numeric(NA))) %>%
  ...
  mutate(total_nan_micro_biomass_score = case_when(
      total_nan_micro_biomass > 133.67 & total_nan_micro_biomass < 426.31 ~ 5,
      (total_nan_micro_biomass >= 131.37 & total_nan_micro_biomass <= 133.67) | (total_nan_micro_biomass >= 426.31 & total_nan_micro_biomass < 685.81) ~ 3,
      total_nan_micro_biomass < 131.37 | total_nan_micro_biomass >= 685.81 ~ 1,
      TRUE ~ as.numeric(NA))) 
```
...
score_spring_m <- metrics_prep.df %>% 
  ...
  mutate(total_nan_micro_biomass_chla_ratio_score = case_when(
       total_nan_micro_biomass_chla_ratio < 45.04 ~ 1,
      total_nan_micro_biomass_chla_ratio >= 45.04 & total_nan_micro_biomass_chla_ratio <= 69.52 ~ 3,
      total_nan_micro_biomass_chla_ratio > 69.52 ~ 5,
      TRUE ~ as.numeric(NA))) %>%
  ...
  mutate(total_nan_micro_biomass_score = case_when(
      total_nan_micro_biomass > 1150 ~ 1,
      total_nan_micro_biomass <= 1150 ~ as.numeric(NA),
      TRUE ~ as.numeric(NA))) #%>%
```
...
score_spring_p <- metrics_prep.df %>% 
  ...
  mutate(total_nan_micro_biomass_chla_ratio_score = case_when(
      total_nan_micro_biomass_chla_ratio < 71.0 ~ 1,
      total_nan_micro_biomass_chla_ratio >= 71.0 & total_nan_micro_biomass_chla_ratio <= 107.5 ~ 3,
      total_nan_micro_biomass_chla_ratio > 107.5 ~ 5,
      TRUE ~ as.numeric(NA))) %>%
  ...
  mutate(total_nan_micro_biomass_score = case_when(
      total_nan_micro_biomass > 1061.7 ~ 1,
      total_nan_micro_biomass <= 1061.7 ~ as.numeric(NA),
      TRUE ~ as.numeric(NA))) 
...
score_summer_f <- metrics_prep.df %>% 
  ...
  mutate(total_nan_micro_biomass_score = case_when(
      total_nan_micro_biomass > 555.7 ~ 1,
      total_nan_micro_biomass < 231.3 ~ 3,
      total_nan_micro_biomass >= 231.3 & total_nan_micro_biomass <= 555.7 ~ 5,
      TRUE ~ as.numeric(NA))) 
```
...
score_summer_m <- metrics_prep.df %>% 
  ...
  mutate(total_nan_micro_biomass_chla_ratio_score = case_when(
      total_nan_micro_biomass_chla_ratio < 32.2 ~ 1,
      total_nan_micro_biomass_chla_ratio >= 32.2 & total_nan_micro_biomass_chla_ratio <= 36.9 ~ 3,
      total_nan_micro_biomass_chla_ratio > 36.9 ~ 5,
      TRUE ~ as.numeric(NA))) %>%
  ...
  mutate(total_nan_micro_biomass_score = case_when(
      total_nan_micro_biomass > 660 ~ 1,
      total_nan_micro_biomass <= 660 ~ as.numeric(NA),
      TRUE ~ as.numeric(NA))) 
...
score_summer_p <- metrics_prep.df %>% 
  ...
  mutate(total_nan_micro_biomass_chla_ratio_score = case_when(
      total_nan_micro_biomass_chla_ratio < 37.7 ~ 1,
      total_nan_micro_biomass_chla_ratio >= 37.7 & total_nan_micro_biomass_chla_ratio <= 74.5 ~ 3,
      total_nan_micro_biomass_chla_ratio > 74.5 ~ 5,
      TRUE ~ as.numeric(NA))) %>%
  ...
  mutate(picoplankton_abundance_score = case_when(
    ...
    mutate(total_nan_micro_biomass_score = case_when(
      total_nan_micro_biomass < 181 | total_nan_micro_biomass > 831 ~ 1,
      (total_nan_micro_biomass >= 181 & total_nan_micro_biomass <= 207) | (total_nan_micro_biomass >= 718 & total_nan_micro_biomass <= 831) ~ 3,
      total_nan_micro_biomass < 718 ~ 5,
      TRUE ~ as.numeric(NA))) 
```
...
scores_ratings_final <- test_all_seasons%>%
    # mutate(ibi_score =  case_when(season == "spring" & salzone == "f" ~ mean(total_nan_micro_biomass_chla_ratio_score, surface_chla_score, cyanophyte_biomass_score, doc_score, pheophytin_score, total_nan_micro_biomass_score, na.rm = T), TRUE~ as.numeric(NA)))
  
  mutate(ibi_score =  case_when(season == "spring" & salzone == "f" ~ (total_nan_micro_biomass_chla_ratio_score + surface_chla_score + cyanophyte_biomass_score + doc_score + pheophytin_score + total_nan_micro_biomass_score) / 6,
                                
                                  season == "spring" & salzone == "o" ~ (total_nan_micro_biomass_chla_ratio_score + surface_chla_score + doc_score + pheophytin_score +  total_nan_micro_biomass_score) / 5,
                                
                                  season == "spring" & salzone == "m" ~ (total_nan_micro_biomass_chla_ratio_score + surface_chla_score + diatom_biomass_score + dinoflagellate_biomass_score + doc_score + pheophytin_score + prorocentrum_minimum_abundance_score + total_nan_micro_biomass_score) / 8,
                                
                                  season == "spring" & salzone == "p" ~ (total_nan_micro_biomass_chla_ratio_score + surface_chla_score + pct_cryptophyte_score + doc_score + pheophytin_score + prorocentrum_minimum_abundance_score + total_nan_micro_biomass_score) / 7,
                                
                                
                                  season == "summer" & salzone == "f" ~ ( surface_chla_score + cyanophyte_biomass_score + diatom_biomass_score + doc_score + microcystis_aeruginosa_abundance_score + pheophytin_score + total_nan_micro_biomass_score) / 7,
                                
                                  season == "summer" & salzone == "o" ~ (surface_chla_score + cyanophyte_biomass_score + diatom_biomass_score + doc_score + pheophytin_score) / 5,
                                
                                  season == "summer" & salzone == "m" ~ (total_nan_micro_biomass_chla_ratio_score + surface_chla_score + dinoflagellate_biomass_score + doc_score + pheophytin_score + picoplankton_abundance_score + total_nan_micro_biomass_score) / 7,
                                
                                  season == "summer" & salzone == "p" ~ (total_nan_micro_biomass_chla_ratio_score +
surface_chla_score + pct_cryptophyte_score + diatom_biomass_score + dinoflagellate_biomass_score + doc_score + pheophytin_score + picoplankton_abundance_score + total_nan_micro_biomass_score) / 9,
